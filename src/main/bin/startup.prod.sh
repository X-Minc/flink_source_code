...
if [ "$YAML_ENV" == "" ]; then
    YAML_ENV="-Drap.log4j.debug=info -Drap.config.datasource=false -Drap.profile=production -Drap.crypto.key=IzxxW5L7UAdFl1huMCrg2TKs6+B/WeTCFCY+h2M2n5c -Ddsb.on=true -Drap.redis.server=192.168.19.4:6379 -Drap.redis.auth.password=CQC7RxN3ND0zhjwMUBXvtwvNSiQFgnq1 -Drap.mvc.config=false -Djava.awt.headless=true -Djava.net.preferIPv4Stack=true -Dfile.encoding=UTF-8 -Drap.es.url=es-cn-v0h0m1w6e0003cjlw.elasticsearch.aliyuncs.com:9200 -Drap.es.username=elastic -Drap.es.password=JB%3nozMCSAdgC3h -Drap.mq.rocket.username=duLCaybnI9oOyOdZ -Drap.mq.rocket.password=RCHE8NY4DkIldIei3O4ub61FwxEaGa -Drap.mq.rocket.topic=topic_bot_data_sync_online -Drap.mq.rocket.namesrvAddr=http://onsaddr.cn-shanghai.mq-internal.aliyuncs.com:8080 -Drap.mq.rocket.groupId=CID_rap_bot_es_online -Drap.mq.rocket.topic.Two=rap_bot_kbs -Drap.mq.rocket.namesrvAddr.Two=http://MQ_INST_1122260953808354_BbzN8VSk.cn-shanghai.mq-internal.aliyuncs.com:8080 -Drap.mq.rocket.groupId.Two=GID_rap_bot_kbs"
fi
